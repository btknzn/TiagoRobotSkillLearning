...
VisionDetect::VisionDetect(ros::NodeHandle nh_)
{
	...
	pub = nh_.advertise<batu_training::objectLocation>("redBottle", 1);
}
...
void VisionDetect::cloud_cb(const sensor_msgs::PointCloud2ConstPtr& input){
	...
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_filter_coke(new pcl::PointCloud<pcl::PointXYZRGB>);



	pcl::ConditionAnd<pcl::PointXYZRGB>::Ptr range_cond(new pcl::ConditionAnd<pcl::PointXYZRGB>());
	std::uint32_t rgb_filter1 = ((std::uint32_t)90	 <<16  | (std::uint32_t)0<<16  | (std::uint32_t)0<<16);
	std::uint32_t rgb_filter2 = ((std::uint32_t)255<<0  | (std::uint32_t)95<<16 | (std::uint32_t)95<<16);

	range_cond->addComparison(pcl::FieldComparison<pcl::PointXYZRGB>::ConstPtr(new pcl::FieldComparison<pcl::PointXYZRGB>("rgb", pcl::ComparisonOps::GT, *reinterpret_cast<float*>(&rgb_filter1))));
	range_cond->addComparison(pcl::FieldComparison<pcl::PointXYZRGB>::ConstPtr(new pcl::FieldComparison<pcl::PointXYZRGB>("rgb", pcl::ComparisonOps::LT, *reinterpret_cast<float*>(&rgb_filter2))));
	pcl::ConditionalRemoval<pcl::PointXYZRGB> condrem;
	condrem.setCondition(range_cond);
	condrem.setInputCloud(cloud_area_filtered3);
	condrem.setKeepOrganized(true);
	condrem.filter(*cloud_filter_coke);
	float x_mean;
	float y_mean;
	float z_mean;
	x_mean = 0;
	...
	for (const auto& point: *cloud_filter_coke){
	...
	//pcl::io::savePCDFileASCII ("wholescene.pcd", *cloud_area_filtered3);
	//pcl::io::savePCDFileASCII ("cola.pcd", *cloud_filter_coke);
