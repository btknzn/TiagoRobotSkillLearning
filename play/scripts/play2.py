...
class neuralNetwork(object):
    ...
    def findBestAction(self,model, train_input, image_current, depth_current):
        action_array = torch.zeros((1,1,12))
        bestaction_array = torch.zeros((1,1,12))
        for j in range(0,12):
            bestactionReward = 0
            for action in [0.10 ,0.01,0, -0.01, -0.10]:
                action_array[0,0,j] = action
                reward = model(train_input.float(),image_current.float(),depth_current.float(),action_array.float())
                rospy.loginfo("reward:"+str(reward))
                rospy.loginfo("bestactionReward:"+str(bestactionReward))
    ...
    def run(self):
        ...
        Model = RLPredictor()
        Model.load_state_dict(torch.load("/home/btknzn/tiago_public_ws/src/batu_training/scripts/rl.pth"))
        self.prepare_robot()
        #self.armMototionINT()
        while not rospy.is_shutdown():
            X1_MIN = 0.04995837
            ...
            #rospy.loginfo(x_current)
            action = self.findBestAction(Model,x_current,image_current,depth_current)
            x_next = x_current + action
            ...
            X12_MIN = -0.7685144
            X12_MAX = 0.0014590342 
            self.arm_joint1_precited = float((x_next[0,0,0].item()))*(X1_MAX-X1_MIN)+X1_MIN
            self.arm_joint2_precited = float((x_next[0,0,1].item()))*(X2_MAX-X2_MIN)+X2_MIN
            self.arm_joint3_precited = float((x_next[0,0,2].item()))*(X3_MAX-X3_MIN)+X3_MIN
            self.arm_joint4_precited = float((x_next[0,0,3].item()))*(X4_MAX-X4_MIN)+X4_MIN
            self.arm_joint5_precited = float((x_next[0,0,4].item()))*(X5_MAX-X5_MIN)+X5_MIN
            self.arm_joint6_precited = float((x_next[0,0,5].item()))*(X6_MAX-X6_MIN)+X6_MIN
            self.arm_joint7_precited = float((x_next[0,0,6].item()))*(X7_MAX-X7_MIN)+X7_MIN
            #rospy.loginfo(str(x_next))
            

            self.armMototion()
            self.gripper_joint1_predicted = float((x_next[0,0,7].item()))*(X8_MAX-X8_MIN)+X8_MIN
            self.gripper_joint2_predicted = float((x_next[0,0,8].item()))*(X9_MAX-X9_MIN)+X9_MIN
            self.action_gripper()
            self.torso_joint_predicted =  float((x_next[0,0,9].item()))*(X10_MAX-X10_MIN)+X10_MIN
            self.action_torso()
            self.torsohead_joint1_predicted = float((x_next[0,0,10].item()))*(X11_MAX-X11_MIN)+X11_MIN
            self.torsohead_joint2_predicted = float((x_next[0,0,11].item()))*(X12_MAX-X12_MIN)+X12_MIN
